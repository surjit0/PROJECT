...
// Holds all contact data
let cmsTable;

// Demo data (if no saved contacts yet)
let cmsTableDemo = {
    'Dave Rapoza': {
        'phone': '766-786-7868',
...
// Will store the original name when editing (important if user changes it)
let selectedOldName = null;

/**
 * Refreshes the contact table in the DOM
 */
let refreshDOMTable = () => {  
    ...
    let enableDisableNewUserModal = (option) => {
        ...
        if(option === 'enable' && !selectedOldName){
            newPersonName.value = '';
    ...
    newPersonSubmitBtn.addEventListener('click', () => {
        ...
        // Simple validation (highlight empty fields)
        document.getElementById('newPersonName').className = newPersonName === '' ? 'input-err' : '';
        document.getElementById('newPersonPhone').className = newPersonPhone === '' ? 'input-err' : '';
        document.getElementById('newPersonAddress').className = newPersonAddress === '' ? 'input-err' : '';
        ...
        if(newPersonName !== '' && newPersonPhone !== '' && newPersonAddress !== '' ) {
            ...
            if (selectedOldName) {
                ...
                if (selectedOldName !== newPersonName) {
                    delete cmsTable[selectedOldName];
                }
            }
    ...
    addNewEntryBtn.addEventListener('click', () => {
        selectedOldName = null; // ensure fresh add
    ...
    for(let i = 0; i < editBtns.length; i++) {
        editBtns[i].addEventListener('click', ($event) => {
            ...
            selectedOldName = nameToEdit; // store original name
